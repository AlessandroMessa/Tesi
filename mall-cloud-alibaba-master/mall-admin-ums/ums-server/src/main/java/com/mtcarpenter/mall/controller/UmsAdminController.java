...
import com.mtcarpenter.mall.dto.UmsAdminLoginParam;
...
public class UmsAdminController {
    ...
    @Autowired 
    private UmsAdminAuthService       authService;
    
    @Autowired 
    private UmsAdminProfileService    profileService;
    
    @Autowired 
    private UmsAdminPermissionService permissionService;
    @Autowired 
    ...
    public CommonResult<UmsAdmin> register(@RequestBody UmsAdminParam umsAdminParam, BindingResult result) {
        UmsAdmin umsAdmin = authService.register(umsAdminParam);
        if (umsAdmin == null) {
            CommonResult.failed();
        }
    ...
    public CommonResult login(@RequestBody UmsAdminLoginParam umsAdminLoginParam, BindingResult result) {
        String token = authService.login(umsAdminLoginParam.getUsername(), umsAdminLoginParam.getPassword());
        if (token == null) {
    ...
    public CommonResult getAdminInfo(Principal principal) {
        ...
        UmsAdmin umsAdmin = profileService.getAdminByUsername(username);
        Map<String, Object> data = new HashMap<>();
        data.put("username", umsAdmin.getUsername());
        data.put("menus", roleService.getMenuList(umsAdmin.getId()));
        data.put("icon", umsAdmin.getIcon());
        List<UmsRole> roleList = adminRoleService.getRoleList(umsAdmin.getId());
    ...
    public CommonResult<CommonPage<UmsAdmin>> list(@RequestParam(value = "keyword", required = false) String keyword,
        ...
        List<UmsAdmin> adminList = profileService.list(keyword, pageSize, pageNum);
    ...
    public CommonResult<UmsAdmin> getItem(@PathVariable Long id) {
        UmsAdmin admin = profileService.getItem(id);
    ...
    public CommonResult update(@PathVariable Long id, @RequestBody UmsAdmin admin) {
        int count = profileService.update(id, admin);
        if (count > 0) {
    ...
    public CommonResult updatePassword(@RequestBody UpdateAdminPasswordParam updatePasswordParam) {
        int status = authService.updatePassword(updatePasswordParam);
        if (status > 0) {
    ...
    public CommonResult delete(@PathVariable Long id) {
        int count = profileService.delete(id);
        if (count > 0) {
            return CommonResult.success(count);
        }
        return CommonResult.failed();
    ...
    public CommonResult updateStatus(@PathVariable Long id, @RequestParam(value = "status") Integer status) {
        ...
        int count = profileService.update(id, umsAdmin);
        if (count > 0) {
            return CommonResult.success(count);
        }
        return CommonResult.failed();
    ...
    public CommonResult updateRole(@RequestParam("adminId") Long adminId,
        ...
        int count = adminRoleService.updateRole(adminId, roleIds);
        if (count >= 0) {
            return CommonResult.success(count);
        }
        return CommonResult.failed();
    ...
    public CommonResult<List<UmsRole>> getRoleList(@PathVariable Long adminId) {
        List<UmsRole> roleList = adminRoleService.getRoleList(adminId);
    ...
    public CommonResult updatePermission(@RequestParam Long adminId,
        ...
        int count = permissionService.updatePermission(adminId, permissionIds);
        if (count > 0) {
            return CommonResult.success(count);
        }
        return CommonResult.failed();
    ...
    public CommonResult<List<UmsPermission>> getPermissionList(@PathVariable Long adminId) {
        List<UmsPermission> permissionList = permissionService.getPermissionList(adminId);
